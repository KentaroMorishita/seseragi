...
export type TourCoverageReport = Readonly<{
  inventoryTopicCount: number
  coveredTopicCount: number
  missingTopics: readonly string[]
  unexpectedTopics: readonly Readonly<{
    topic: string
    lessonId: string
  }>[]
...
export function validateTourCurriculum(
  ...
  const coverage = tourCoverageReport(curriculum)
  if (coverage.missingTopics.length > 0) {
    throw new Error(
      `Tour required topic(s) missing: ${coverage.missingTopics.join(", ")}`
    )
  }
  if (coverage.unexpectedTopics.length > 0) {
    throw new Error(
      `Tour introduced topic(s) missing from requiredTopics: ${coverage.unexpectedTopics.map(({ topic }) => topic).join(", ")}`
    )
  }
  const lessonOrderById = new Map(
    lessons.map(({ id, order }) => [id, order] as const)
  )
  for (const lesson of lessons) {
    assertUnique(
      `Tour lesson ${lesson.id} introduced surface`,
      lesson.introducedSurfaces
    )
    assertUnique(
      `Tour lesson ${lesson.id} required surface`,
      lesson.requiredSurfaces
    )
    for (const requiredSurface of lesson.requiredSurfaces) {
      const introducingLessonId = introducedBy.get(requiredSurface)
      if (introducingLessonId === undefined) {
        throw new Error(
          `Tour lesson ${lesson.id} requires surface ${requiredSurface}, but no lesson introduces it`
        )
      }
      if (lessonOrderById.get(introducingLessonId)! >= lesson.order) {
        throw new Error(
          `Tour lesson ${lesson.id} requires surface ${requiredSurface} before ${introducingLessonId} introduces it`
        )
      }
    }
  }
...
export function tourCoverageReport(
  curriculum: TourCurriculum
): TourCoverageReport {
  const lessons = tourCurriculumLessons(curriculum)
  const introducedBy = new Map<string, CurriculumLesson>()
  for (const lesson of lessons) {
    for (const topic of lesson.introducedSurfaces) {
      if (!introducedBy.has(topic)) introducedBy.set(topic, lesson)
    }
  }
  const requiredTopics = new Set(curriculum.requiredTopics)
  const topics = curriculum.requiredTopics.flatMap((topic) => {
    const lesson = introducedBy.get(topic)
    return lesson === undefined
      ? []
      : [{ topic, lessonId: lesson.id, lessonOrder: lesson.order }]
  })
  return {
    inventoryTopicCount: curriculum.requiredTopics.length,
    coveredTopicCount: topics.length,
    missingTopics: curriculum.requiredTopics.filter(
      (topic) => !introducedBy.has(topic)
    ),
    unexpectedTopics: [...introducedBy.entries()]
      .filter(([topic]) => !requiredTopics.has(topic))
      .map(([topic, lesson]) => ({ topic, lessonId: lesson.id })),
    topics,
  }
}

export function renderTourCoverageReport(curriculum: TourCurriculum): string {
  const report = tourCoverageReport(curriculum)
  const difference =
    report.missingTopics.length === 0 && report.unexpectedTopics.length === 0
      ? "No differences."
      : [
          `Missing from lessons: ${report.missingTopics.join(", ") || "none"}`,
          `Missing from inventory: ${report.unexpectedTopics.map(({ topic }) => topic).join(", ") || "none"}`,
        ].join("\n")
  return [
    ...
    "",
    `Coverage: ${report.coveredTopicCount}/${report.inventoryTopicCount} required compiler surfaces.`,
    "",
    "## Difference",
    "",
    difference,
...
function validatePrerequisiteGraph(lessons: readonly CurriculumLesson[]): void {
  ...
  const root = lessons[0]
  if (root === undefined) return
  const reachable = new Set([root.id])
  for (const lesson of lessons.slice(1)) {
    if (lesson.prerequisites.some((id) => reachable.has(id))) {
      reachable.add(lesson.id)
    }
  }
  const unreachable = lessons.filter(({ id }) => !reachable.has(id))
  if (unreachable.length > 0) {
    throw new Error(
      `Tour lesson(s) unreachable from canonical root ${root.id}: ${unreachable.map(({ id }) => id).join(", ")}`
    )
  }
}
...
function parseLesson(value: unknown, index: number): CurriculumLesson {
  ...
  const focus = expectPlainTexts(record.focus, `${label}.focus`)
  if (focus.length < 1 || focus.length > 2) {
    throw new Error(`${label}.focus must contain one or two central concepts`)
  }
  return {
