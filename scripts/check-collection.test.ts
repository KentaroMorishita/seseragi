...
test("full gate collects early and nested failures and still runs final packaging", () => {
...
case "$*" in
  *check-readme.ts*|*samples:check*) exit 7 ;;
esac
exit 0
`
    ...
    const result = Bun.spawnSync(["bash", "scripts/check-scoped.sh", "full"], {
      cwd: root,
      env: {
        ...process.env,
