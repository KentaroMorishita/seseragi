...
import { 
  Just, Nothing, mapMaybe, pureMaybe, applyMaybe, bindMaybe,
  Left, Right, mapEither, pureEither, applyEither, bindEither,
  type Maybe, type Either
...
test('Applicative Laws - Maybe - Interchange', () => {
  ...
  const right = applyMaybe(pureMaybe((g: (a: number) => number) => g(x)), u);
...
test('Either - Error propagation', () => {
  ...
  const result = bindEither(
    bindEither(parseNum("abc"), double),
    x => Right(x + 1)
  );
...
test('Applicative apply operator - Maybe', () => {
  ...
  const failureResult = applyMaybe(applyMaybe(pureMaybe(multiply), Nothing), Just(3));
  const allNothingResult = applyMaybe(applyMaybe(pureMaybe(add), Nothing), Nothing);
...
test('Applicative apply operator - Either', () => {
  ...
  const successResult = applyEither(applyEither(pureEither(add), Right(10)), Right(5));
  const firstErrorResult = applyEither(applyEither(pureEither(subtract), Left("error1")), Right(5));
  const secondErrorResult = applyEither(applyEither(pureEither(add), Right(10)), Left("error2"));
  const bothErrorResult = applyEither(applyEither(pureEither(add), Left("error1")), Left("error2"));
...
test('Mixed monad operations - Maybe chain', () => {
  ...
  const result = bindMaybe(
    mapMaybe(
      applyMaybe(applyMaybe(pureMaybe(add), input1), input2),
      double
    ),
    x => safeDiv(x)(2)
  );
...
test('Mixed monad operations - Either chain', () => {
  ...
  const result = bindEither(
    mapEither(
      applyEither(applyEither(pureEither(add), safeParse("10")), safeParse("5")),
      double
    ),
    x => Right(x + 1)
  );
  ...
  const errorResult = bindEither(
    mapEither(
      applyEither(applyEither(pureEither(add), safeParse("abc")), safeParse("5")),
      double
    ),
    x => Right(x + 1)
  );
