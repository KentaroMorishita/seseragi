...
const actionButton = (id: string) => (text: string) => (action: Action) => _ssrg_html_button(({ "id": id, "onClick": action, "class": "w-full rounded-xl border-0 px-4 py-3 font-semibold transition hover:shadow-lg", "style": actionStyle(action), "children": text } as const))
const view = (mode: Mode) => _ssrg_html_main(({ "style": pageStyle, "children": [_ssrg_html_section(({ "class": "mx-auto max-w-lg", "style": cardStyle, "children": [_ssrg_html_div(({ "class": "mb-6 flex items-center justify-between", "children": [_ssrg_html_span(({ "style": badgeStyle(mode), "children": label(mode) } as const)), _ssrg_html_span(({ "class": "text-sm font-semibold text-slate-500", "children": "SESERAGI FLOW" } as const))] } as const)), _ssrg_html_h1(({ "class": "m-0 text-3xl font-bold text-slate-900 sm:text-4xl", "children": title(mode) } as const)), _ssrg_html_p(({ "class": "mb-6 mt-4 text-lg text-slate-700", "children": guidance(mode) } as const)), _ssrg_html_div(({ "style": trackStyle, "children": _ssrg_html_div(({ "style": progressStyle(mode), "children": "" } as const)) } as const)), _ssrg_html_div(({ "class": "mt-6 grid grid-cols-1 gap-2 sm:grid-cols-3", "children": [actionButton("focus")("Start focus")(StartFocus), actionButton("break")("Take a break")(TakeBreak), actionButton("reset")("Reset the flow")(Reset)] } as const))] } as const))] } as const))
