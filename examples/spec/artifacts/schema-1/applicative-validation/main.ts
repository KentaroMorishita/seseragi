...
export type User =
  | { readonly tag: "User"; readonly value: readonly [string, number] };
export const User = (value: readonly [string, number]): User => ({ tag: "User", value } as const);
...
const validateUser = (name: string) => (age: number) => ((__ssrg$instance$Applicative$1<FormError>(__ssrg$instance$Functor$0<FormError>())["apply"](((__ssrg$instance$Applicative$1<FormError>(__ssrg$instance$Functor$0<FormError>())["apply"](((__ssrg$instance$Applicative$1<FormError>(__ssrg$instance$Functor$0<FormError>())["pure"](makeUser)) as Validation<FormError, (argument: string) => (argument: number) => User>))(validateName(name))) as Validation<FormError, (argument: number) => User>))(validateAge(age))) as Validation<FormError, User>)
...
export const main = (_unit: undefined) => _ssrg_effect_flatMap(_ssrg_console_println(render(validateUser("")(0))), () => _ssrg_console_println(render(validateUser("Mio")(20))))
