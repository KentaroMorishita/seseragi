...
const userId = (value: bigint) => value
...
const result = (value: { readonly tag: "Left"; readonly value: AppError } | { readonly tag: "Right"; readonly value: bigint }) => value
const increment = (state: MutableSignal<bigint>) => _ssrg_signal_update((value: bigint) => _ssrg_int64_add(value, 1n), state)
const view = (state: MutableSignal<bigint>) => (current: bigint) => _ssrg_html_button(({ "onClick": increment(state), "children": _ssrg_show_intShow["show"](current) } as const))
const content = (state: MutableSignal<bigint>) => _ssrg_signal_map(view(state), state)
const compact = (_unit: undefined) => _ssrg_effect_succeed(undefined)
export const main = (_unit: undefined) => _ssrg_effect_flatMap(_ssrg_signal_make<bigint>(41n), (state: MutableSignal<bigint>) => _ssrg_effect_flatMap(increment(state), () => (() => { const rendered: Signal<Html<Effect<{  }, never, undefined>>> = content(state); return _ssrg_effect_flatMap(_ssrg_signal_read(state), (current: bigint) => (() => { const values: { readonly "left": bigint; readonly "right": bigint } = pair(apply(userId)(current)); return _ssrg_console_println("aliases: " + _ssrg_show_intShow["show"]((values)["left"]) + ", signal: " + _ssrg_show_intShow["show"](current)); })()); })()))
