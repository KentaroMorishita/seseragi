...
const totalsAreCorrect = (unit: undefined) => (($ssrg_match: readonly [number, number]): boolean => $ssrg_match[0] === 45 && $ssrg_match[1] === 55 ? true : false)([_ssrg_range_reduce(0, (((_argument0) => (_argument1) => _ssrg_int_add(_argument0, _argument1)) as (argument: number) => (argument: number) => number), exclusiveValues(undefined)), _ssrg_range_reduce(0, (((_argument0) => (_argument1) => _ssrg_int_add(_argument0, _argument1)) as (argument: number) => (argument: number) => number), inclusiveValues(undefined))] as const)
