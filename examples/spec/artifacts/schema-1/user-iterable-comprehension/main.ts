...
const advance = (limit: number) => (current: number) => (_ssrg_int_ord_dictionary["compare"](current)(limit))["tag"] !== "Greater" ? _ssrg_maybe_Just([current, _ssrg_int_add(current, 1)] as const) : _ssrg_maybe_Nothing
...
const totals = (values: Countdown) => [_ssrg_array_reduce(0, (((_argument0) => (_argument1) => _ssrg_int_add(_argument0, _argument1)) as (argument: number) => (argument: number) => number), oddSquares(values)), ((total(values)(__ssrg$instance$Reducible$1(__ssrg$instance$Iterable$0))) as number)] as const
