...
export type Envelope<A> = {
  ...
  readonly "value": A;
  readonly "range": Readonly<{ start: number; end: number; inclusive: boolean }>;
  readonly "summary": readonly [number, string];
...
export const main = (_unit: undefined) => _ssrg_effect_flatMap(_ssrg_console_println(((__ssrg$instance$Show$2<UserId>(__ssrg$instance$Show$0)["show"](packet)) as string)), () => _ssrg_effect_flatMap(_ssrg_console_println(((__ssrg$instance$Debug$5<Packet<UserId>>(__ssrg$instance$Debug$3<UserId>(__ssrg$instance$Debug$1))["debug"](envelope)) as string)), () => _ssrg_effect_flatMap(_ssrg_console_println(((__ssrg$instance$Debug$6["debug"](stats)) as string)), () => _ssrg_effect_flatMap(_ssrg_console_println(((__ssrg$instance$Show$0["show"](UserId(42))) as string)), () => _ssrg_console_println(((__ssrg$instance$Show$2<number>(_ssrg_show_intShow)["show"](empty)) as string))))))
const packet: Packet<UserId> = Packet([UserId(7), UserId(9)]);
const empty: Packet<number> = Empty;
const envelope: Envelope<Packet<UserId>> = (({ "label": "metrics", "value": packet, "range": _ssrg_range_inclusive(1, 3), "summary": [42, "ready"] as const, "metadata": ({ "active": true } as const) } as const) as unknown as Envelope<Packet<UserId>>);
const stats: Stats = (({ "total": _ssrg_int_add(20, 22), "ratio": 1.25 } as const) as unknown as Stats);
