...
export const __ssrg$instance$Render$1 = <T,>(__ssrg$evidence$0: unknown) => ({ "render": (value: { readonly tag: "Nothing" } | { readonly tag: "Just"; readonly value: T }) => (($ssrg_match: { readonly tag: "Nothing" } | { readonly tag: "Just"; readonly value: T }): string => $ssrg_match.tag === "Nothing" ? "Constrained dictionary: empty" : $ssrg_match.tag === "Just" ? ((item: T): string => __ssrg$evidence$0["ready"](item))($ssrg_match.value) : ((): never => { throw new Error("non-exhaustive Seseragi match"); })())(value) }) as const;
