...
const update = (action: CounterAction) => (state: CounterState) => (($ssrg_match: CounterAction): CounterState => $ssrg_match.tag === "Increment" ? (({ "count": _ssrg_int_add((state)["count"], 1) } as const) as unknown as CounterState) : (({ "count": _ssrg_int_subtract((state)["count"], 1) } as const) as unknown as CounterState))(action)
