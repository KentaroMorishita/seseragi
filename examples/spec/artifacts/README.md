...
後続local functionへのforward referenceをcompile diagnosticとして固定します。相互再帰`rec` groupは
このcaseに含みません。`schema-1/self-tail-loop`と`execution-schema-1/self-tail-loop`はdirect self tail callを
...
`schema-1/fizzbuzz`は3と5のremainderをTuple patternで分類し、Range comprehensionから1..=30のArrayを作ります。
