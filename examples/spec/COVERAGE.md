...
| nominal Struct value / update / pattern / module boundary | Lesson 11 | `schema-1/{struct-profile,generic-struct}` + `execution-schema-1/{struct-profile,generic-struct}` + `semantic-diagnostics-schema-1/struct-field-errors` + `project-schema-1/imported-struct`; explicit / shorthand construction、field / expected / spreadからのgeneric argument inference、same-owner / same-argument spread update、member access、named struct pattern、public field contract、type-only import、unique-symbol brand、cross-module actual executionを接続。明示type argument construction、opaque smart constructor / field visibility、struct deriving / implは未接続 | partial |
...
| generic struct / alias / impl                 | Lesson 11         | `schema-1/generic-struct`がfield inference、generic access / pattern / update、`Box<bigint>`生成とactual executionを固定。明示type argument construction / alias / implはnested generic / kind fixtureへ続く | partial |
