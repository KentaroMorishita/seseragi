...
| structural record value / optional field / presence | Lesson 27  | `schema-1/record-profile` + `execution-schema-1/record-profile` + `semantic-diagnostics-schema-1/record-field-errors` + `compile/optional-record-field.ssrg`; explicit / shorthand / spread literal、required access、optional accessのMaybe presence、width subtyping、readonly TS object、duplicate / missing / invalid spread diagnosticを接続。record patternは未接続 | partial |
