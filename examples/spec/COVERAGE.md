...
| newtype、deriving、operator overload          | Lesson 07         | `schema-1/{pure-comparison,string-add,user-add-operator,user-eq-operator}` + `execution-schema-1/{pure-comparison-string,string-add,user-add-operator,user-eq-operator}` + `semantic-diagnostics-schema-1/{user-add-missing,operator-reference-missing,eq-missing}` + `project-schema-1/{imported-user-add-operator,imported-user-eq-operator}`; standard / local / imported / scoped `Add` / `Eq` evidence、binary / contextual operator section dictionary dispatch、`!=`のEq否定、user `Add` callbackによるArray reduce。generalized constrained value scheme、struct / newtype糖衣、deriving Eqは未接続 | partial |
