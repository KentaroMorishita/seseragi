...
| Monad、`>>=`、do desugar                      | Lesson 10         | `schema-1/{monad-maybe,monad-either,array-monad,list-monad,effect-monad,monad-laws}` + execution fixtures + `semantic-diagnostics-schema-1/monad-do-invalid` + `project-schema-1/imported-generic-adt-monad`; transitive supertrait、generic `pure` / `flatMap`、Prelude Maybe / Either / Array / List / Effect標準dictionary、明示lambdaによる`>>=`とdoの同値execution、dictionary-driven pure do、first-class Effect value、userland型クラス定義sample、imported user ADTのbind / pure do、do固有diagnostic、left / right identityとassociativity law regression | partial |
...
| rank-1 generic / higher-order pure function / operator section / module import | Lesson 11 | `schema-1/{generic-higher-order-call,operator-reference,custom-operator-section}` + `execution-schema-1/custom-operator-section` + imported custom operator lowering test + `project-schema-1/rock-paper-scissors-domain-split`; arithmetic / custom operatorをcurried function値としてhigher-order callと部分適用へ接続。comparison / class operator sectionは後続 | partial |
