...
| Array/List、Iterator、lambda、Either          | Lessons 04 and 08 | `schema-1/{array-literal,iterator-unfold}` + `execution-schema-1/iterator-unfold`; persistent lazy `unfold` / `next` runtime ABI + collection boundary fixtures | partial |
...
| comprehension、Array / record pattern         | Lesson 12         | `schema-1/{range-comprehension,array-comprehension,comprehension-pattern-filter,comprehension-invalid-source}` + `execution-schema-1/{range-comprehension,comprehension-pattern-filter}`; Array / Range `Iterable<C, A>` evidence、guard、multiple generator、constructor / tuple pattern filter | partial |
