...
| Array/List、Iterator、lambda、Either          | Lessons 04 and 08 | `schema-1/{array-literal,list-literal,iterator-unfold,user-iterable-comprehension}` + `execution-schema-1/{iterator-unfold,user-iterable-comprehension}`; persistent `Empty / Cons` List literal、lazy `unfold` / `next` runtime ABI + user `Iterable` collection boundary fixtures | partial |
