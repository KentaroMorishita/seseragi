...
| HKT / `Functor<F<_>>` / generic `map`          | Lessons 09-11     | `schema-1/{functor-maybe,type-constructor-kind-mismatch,monad-maybe,monad-either}` + execution fixtures; kind arity、constructor inference、部分適用`Either<E, _>`、parameter / local evidence、`<$>` dictionary execution | partial |
| ApplicativeとValidation accumulation          | Lesson 09         | `schema-1/{applicative-maybe,monad-maybe,monad-either}` + execution fixtures; Functor supertrait、`pure` / `apply` / inherited `map`、部分適用constructorの`<*>` dictionary execution。law / validation accumulationは未接続 | partial |
| Monad、`>>=`、do desugar                      | Lesson 10         | `schema-1/{monad-maybe,monad-either}` + execution fixtures + `semantic-diagnostics-schema-1/monad-do-invalid`; transitive supertrait、generic `pure` / `flatMap`、`>>=`、dictionary-driven pure do、Maybe / Either short-circuit、do固有diagnostic。lawは未接続 | partial |
