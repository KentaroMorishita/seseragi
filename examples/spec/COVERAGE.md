...
| Monad、`>>=`、do desugar                      | Lesson 10         | `schema-1/{monad-maybe,monad-either,array-monad,list-monad,monad-laws}` + execution fixtures + `semantic-diagnostics-schema-1/monad-do-invalid` + `project-schema-1/imported-generic-adt-monad`; transitive supertrait、generic `pure` / `flatMap`、Prelude Maybe / Either / Array / List標準dictionary、明示lambdaによる`>>=`とdoの同値execution、dictionary-driven pure do、userland型クラス定義sample、imported user ADTのbind / pure do、do固有diagnostic、left / right identityとassociativity law regression | partial |
