...
| Array/List、Iterator、lambda、Either          | Lessons 04 and 08 | `schema-1/{array-literal,iterator-unfold,user-iterable-comprehension}` + `execution-schema-1/{iterator-unfold,user-iterable-comprehension}`; persistent lazy `unfold` / `next` runtime ABI + user `Iterable` collection boundary fixtures | partial |
...
| HKT / `Functor<F<_>>` / generic `map`          | Lessons 09-11     | `schema-1/{functor-maybe,partial-functor-value,type-constructor-kind-mismatch,monad-maybe,monad-either,monad-laws}` + execution fixtures; kind arity、constructor inference、期待関数型からのpartial trait method evidence、部分適用`Either<E, _>`、parameter / local evidence、`<$>` dictionary execution、identity / composition law regression | partial |
