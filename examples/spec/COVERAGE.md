...
| ApplicativeとValidation accumulation          | Lesson 09         | `schema-1/{applicative-maybe,monad-maybe,monad-either,monad-laws}` + execution fixtures + `project-schema-1/imported-generic-adt-monad`; Functor supertrait、`pure` / `apply` / inherited `map`、部分適用constructorとimported user ADTのfactory materialization、`<*>` dictionary execution、identity / homomorphism law regression。validation accumulationは未接続 | partial |
