...
Object.assign(__variableTypes, {
  ...
  "result3": "Maybe<unknown>",
  "result4": "Maybe<unknown>",
  "rightX": "Either<unknown, Int>",
  "rightY": "Either<unknown, Int>",
  "leftError": "Either<String, unknown>",
  "eitherResult1": "Either<unknown, unknown>",
  "eitherResult2": "Either<unknown, unknown>",
  "eitherResult3": "Either<unknown, unknown>",
  "eitherResult4": "Either<unknown, unknown>",
  "eitherResult5": "Either<unknown, unknown>",
  "accessResult1": "Maybe<unknown>",
  "accessResult2": "Maybe<unknown>",
  "accessResult3": "Maybe<unknown>",
  "userResult1": "Either<unknown, unknown>",
  "userResult2": "Either<unknown, unknown>",
  "userResult3": "Either<unknown, unknown>",
  "compose": "function",
  "doubleAndIncrement": "function",
  "squareAndDouble": "function",
  "report1": "Either<String, unknown>",
  "report2": "Either<String, unknown>",
  "report3": "Either<String, unknown>"
});
...
const result3 = (() => {
      ...
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, doubleIfPositive);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, doubleIfPositive);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, doubleIfPositive);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, doubleIfPositive);
      } else {
        return bindMaybe(_monad, doubleIfPositive);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
...
const eitherResult3 = (() => {
      ...
      const _monad = rightX;
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, multiplyBy2);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, multiplyBy2);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, multiplyBy2);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, multiplyBy2);
      } else {
        return bindMaybe(_monad, multiplyBy2);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, subtractFive);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, subtractFive);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, subtractFive);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, subtractFive);
      } else {
        return bindMaybe(_monad, subtractFive);
      }
    })();

const eitherResult4 = (() => {
      const _monad = (() => {
      const _monad = Right(0);
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, multiplyBy2);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
...
const accessResult1 = (() => {
      const _monad = (() => {
      const _monad = (() => {
      const _monad = Just("user123");
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, parseUserId);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, parseUserId);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, parseUserId);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, parseUserId);
      } else {
        return bindMaybe(_monad, parseUserId);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, checkPermission);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, checkPermission);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, checkPermission);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, checkPermission);
      } else {
        return bindMaybe(_monad, checkPermission);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, accessResource);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, accessResource);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, accessResource);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, accessResource);
      } else {
        return bindMaybe(_monad, accessResource);
      }
    })();
...
const userResult1 = (() => {
      const _funcs = (() => {
      const _funcs = (() => {
      const _funcs = Right(createUser);
      const _values = validateName("Alice");
      if (Array.isArray(_funcs) && Array.isArray(_values)) {
        return applyArray(_funcs, _values);
      } else if (_funcs && (_funcs.tag === 'Cons' || _funcs.tag === 'Empty') &&
                _values && (_values.tag === 'Cons' || _values.tag === 'Empty')) {
      ...
      const _values = validateEmail("alice@example.com");
      if (Array.isArray(_funcs) && Array.isArray(_values)) {
        return applyArray(_funcs, _values);
      } else if (_funcs && (_funcs.tag === 'Cons' || _funcs.tag === 'Empty') &&
                _values && (_values.tag === 'Cons' || _values.tag === 'Empty')) {
...
const userResult2 = (() => {
      ...
      const _funcs = (() => {
      const _funcs = Right(createUser);
      const _values = validateName("admin");
      if (Array.isArray(_funcs) && Array.isArray(_values)) {
        return applyArray(_funcs, _values);
      } else if (_funcs && (_funcs.tag === 'Cons' || _funcs.tag === 'Empty') &&
                _values && (_values.tag === 'Cons' || _values.tag === 'Empty')) {
...
const userResult3 = (() => {
      ...
      const _funcs = (() => {
      const _funcs = Right(createUser);
      const _values = validateName("Bob");
      if (Array.isArray(_funcs) && Array.isArray(_values)) {
        return applyArray(_funcs, _values);
      } else if (_funcs && (_funcs.tag === 'Cons' || _funcs.tag === 'Empty') &&
                _values && (_values.tag === 'Cons' || _values.tag === 'Empty')) {
      ...
      const _values = validateEmail("test@test.com");
      if (Array.isArray(_funcs) && Array.isArray(_values)) {
        return applyArray(_funcs, _values);
      } else if (_funcs && (_funcs.tag === 'Cons' || _funcs.tag === 'Empty') &&
                _values && (_values.tag === 'Cons' || _values.tag === 'Empty')) {
...
const report1 = (() => {
      const _monad = (() => {
      const _monad = (() => {
      const _monad = fetchUserData("user123");
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, parseScore);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, parseScore);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, parseScore);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, parseScore);
      } else {
        return bindMaybe(_monad, parseScore);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, calculateBonus);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, calculateBonus);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, calculateBonus);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, calculateBonus);
      } else {
        return bindMaybe(_monad, calculateBonus);
      }
    })();
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, generateReport);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
        return bindArray(_monad, generateReport);
      } else if (_monad && (_monad.tag === 'Cons' || _monad.tag === 'Empty')) {
        return bindList(_monad, generateReport);
      } else if (_monad && (_monad.tag === 'Left' || _monad.tag === 'Right')) {
        return bindEither(_monad, generateReport);
      } else {
        return bindMaybe(_monad, generateReport);
      }
    })();
...
const report2 = (() => {
      const _monad = (() => {
      const _monad = (() => {
      const _monad = fetchUserData("user456");
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, parseScore);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
...
const report3 = (() => {
      const _monad = (() => {
      const _monad = (() => {
      const _monad = fetchUserData("unknown");
      if (_monad && _monad.tag === 'Tuple') {
        const results = bindArray(_monad.elements, parseScore);
        return { tag: 'Tuple', elements: results };
      } else if (Array.isArray(_monad)) {
