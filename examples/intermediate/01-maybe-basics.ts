...
console.log("=== Maybe型の基本 ===");

console.log("--- Maybe型とは ---");
...
const numberToMonth = (n: number): Maybe<string> => ((n === 1) ? Just("January") : ((n === 2) ? Just("February") : ((n === 3) ? Just("March") : ((n === 4) ? Just("April") : ((n === 5) ? Just("May") : ((n === 6) ? Just("June") : ((n === 7) ? Just("July") : ((n === 8) ? Just("August") : ((n === 9) ? Just("September") : ((n === 10) ? Just("October") : ((n === 11) ? Just("November") : ((n === 12) ? Just("December") : Nothing))))))))))));

show(numberToMonth(0));

show(numberToMonth(1));

show(numberToMonth(12));

show(numberToMonth(13));

const safeGet = (index: number): Maybe<number> => ((index < 0) ? Nothing : ((index >= 10) ? Nothing : Just((index * 10))));
...
const parseAge = (input: string): Maybe<number> => ((input === "25") ? Just(25) : ((input === "30") ? Just(30) : Nothing));
...
const findUser = (id: number): Maybe<string> => ((id === 1) ? Just("Alice") : ((id === 2) ? Just("Bob") : Nothing));
...
console.log("--- Maybe型の利点 ---");

console.log("Maybe型により、値の存在/非存在を型レベルで表現できます");
