...
├── 24-match-examples.ssrg       # 🌟 ADT/Match機能の完全サンプル
├── fizzbuzz.ssrg               # FizzBuzz実装例
├── 01-advanced-features.ssrg    # 高度な機能
├── 02-tutorial.ssrg             # 基本チュートリアル
...
├── 09-range-comprehension.ssrg  # レンジとリスト内包表記
├── 10-ternary-operator.ssrg     # 三項演算子
├── 11-tuple-basics.ssrg         # タプル基本操作
├── 12-struct-basics.ssrg        # 構造体基本操作
├── 13-struct-impl-basic.ssrg    # 構造体メソッド実装
├── 14-struct-field-access.ssrg  # 構造体フィールドアクセス
├── 15-struct-arithmetic-issue.ssrg # 構造体算術演算
├── 16-struct-implicit-self.ssrg # 構造体の暗黙的self
├── 17-struct-float-arithmetic.ssrg # 構造体浮動小数点演算
├── 18-method-calls-and-operators.ssrg # メソッド呼び出しと演算子
├── 19-simple-method-test.ssrg   # シンプルなメソッドテスト
├── 20-array-monad-operators.ssrg # 配列モナド演算子
├── 21-array-list-monad-comparison.ssrg # 配列とリストのモナド比較
├── 22-spread-destructuring.ssrg # スプレッド構文と分割代入
├── 23-array-tuple-index-access.ssrg # 配列・タプルのインデックスアクセス
└── 24-match-examples.ssrg       # ADT/Match機能の完全サンプル
```

## サンプルファイル

### 🌟 `24-match-examples.ssrg` - ADT/Match機能の完全サンプル（推奨開始点）

ADT（代数データ型）とパターンマッチング機能の包括的なサンプル。
- 基本的なADT定義とマッチング
- ビルトインMaybe/Either/List型の直接使用
- 実践的なパターンマッチング例
