...
性能表記では、`n`と`m`を入力の要素数、byte数、または本文で指定した単位、`k`を出力sizeとします。
特記しない`O(...)`はworst-caseです。`expected`はprocess-local hash seedの分布に対して評価し、
一runの途中でseedは変えません。`amortized`は同じ値へ連続して操作した列全体で評価します。
user callback、`Eq`、`Ord`、`Hash`の
内部実行時間は計算量へ含めず、呼び出し回数を別に数えます。

strictな結果を作るoperationは、明記したsharingを除き、結果sizeに比例するstorageを持ちます。immutableな
入力そのものを返してよいcaseではallocationを省略できます。allocationの個数やhost object layoutは
観測保証ではありませんが、明記したsharing contractを除き、結果から論理的に到達できないuser valueを
backing storageやtombstone経由で保持し続けてはなりません。sharingを許すAPIは、保持範囲と独立storageを
作る方法を本文で定めます。
...
Arrayの結果は、結果に含まれない入力要素をbacking storage経由で保持しません。view表現を内部で使う場合も、
小さい`take`、`drop`、`init`、`tail`、`windows`の結果だけで入力全体が到達可能なままにならないよう
compact化しなければなりません。Listは結果に含まれるsuffix nodeを共有できますが、結果より前や後にある
nodeを隠れて保持しません。この規則を満たすためのcopyは意味上観測できず、専用copy APIを要求しません。
...
適切な`Hash` instanceのもとで、`get`、`containsKey`、`insert`、`upsert`、`remove`はexpected O(1)回の
hash-index操作とO(1)回の`Hash`呼び出しです。hash collision時の`Eq`回数とworst-caseはO(n)です。
`fromEntries`、`filter`、`mapValues`、`mapKeysWith`、`keys`、`values`、`entries`は入力と出力に対して
O(n)、`mergeWith`はexpected O(n + m)です。callbackは各規則で指定した回数だけ呼びます。

Mapはpersistentで、point updateは変更経路以外を共有でき、expected O(1)の追加storageです。collisionが
一bucketへ集中した場合はO(n)まで許します。結果Map単体から到達する内部storageはO(size)で、remove後の
結果は削除したkeyとvalueを順序tombstoneやindex entryから保持しません。古いMap値自体が生存している場合、
その値が持つentryまで解放することは要求しません。
...
適切な`Hash` instanceのもとで、`contains`、`insert`、`remove`はexpected O(1)回のhash-index操作です。
`fromIterable`、`filter`、`map`、`toArray`、`toList`は入力と出力に対してexpected O(n)、`union`、
`intersection`、`difference`、`isSubsetOf`はexpected O(n + m)です。collision時のworst-caseはO(n \* m)を
許します。point updateの追加storage、結果全体のstorage、削除したuser valueの保持はMapと同じ規則です。
...
substringを返すoperationはimmutable backing storageを内部共有できますが、返したStringやString配列から
論理的な結果より漸近的に大きい入力storageを保持してはなりません。したがって、小さい`sliceScalars`、
match capture、split part、line、wordだけを保持しても元の巨大String全体を生存させません。Stringには
storage identityがなく、この保証を得るための明示copy APIは提供しません。
...
Intから構築します。hostのnumberへ精度を落として実装してはなりません。

```seseragi
type BigIntParseError deriving Eq, Show =
  | EmptyBigInt
  | InvalidBigIntRadix Int
  | InvalidBigIntDigit { offset: Int, radix: Int }

type BigIntDivisionError deriving Eq, Show =
  | BigIntDivisionByZero

type BigIntPowerError deriving Eq, Show =
  | NegativeBigIntExponent Int

type BigIntConversionError deriving Eq, Show =
  | BigIntOutsideIntRange

fn parse text: String -> Either<BigIntParseError, BigInt>
fn parseRadix radix: Int -> text: String -> Either<BigIntParseError, BigInt>
fn format value: BigInt -> String
fn formatRadix radix: Int -> value: BigInt
  -> Either<BigIntParseError, String>

fn fromInt value: Int -> BigInt
fn toInt value: BigInt -> Either<BigIntConversionError, Int>
fn checkedDivide divisor: BigInt -> dividend: BigInt
  -> Either<BigIntDivisionError, BigInt>
fn checkedRemainder divisor: BigInt -> dividend: BigInt
  -> Either<BigIntDivisionError, BigInt>
fn checkedPower exponent: Int -> base: BigInt
  -> Either<BigIntPowerError, BigInt>
fn abs value: BigInt -> BigInt
fn sign value: BigInt -> Int
```
...
`dividend == quotient * divisor + remainder`を満たし、0でないremainderはdividendと同符号です。
operatorの0除算と負指数は9.6のdefectで、通常値として扱う場合はchecked APIを使います。exponent zeroは
baseによらず1で、`0 ** 0`も1です。

値のbit lengthを`n`、`m`、出力のbit lengthを`k`とすると、保持storage、Eq、Ord、HashはO(n)、
加減算はO(n + m)、乗算と除算はworst-case O(n \* m)を上限とします。powerはexponentiation by squaring
相当のO(log exponent)回の乗算で実装できなければなりません。parse / formatは入力・出力digit数`d`に
対してworst-case O(d^2)、結果storageはO(k)です。より高速な乗算・除算・基数変換は同じ意味を保つ
quality of implementationです。
...
hexとbase64のencode / decodeは入力と出力のbyte数に対してO(n + k)で、結果sizeに比例するstorageを持ちます。
...
各operationは入力のUTF-8 byte数に対してO(n)で、`clusters`と`byteBoundaries`は出力sizeに比例するstorageを
追加で持ちます。返すcluster、`at`、`slice`のStringは10.7のsubstring storage規則に従います。
...
parse、stringify、standard codecは入力Json node、text byte、出力sizeに対して線形です。user-defined
decoderが`oneOf`や`flatMap`で同じsubtreeを複数回調べるcostは、そのdecoder自身の合成に従います。
parseとdecodeはfailure位置より後を検証する必要がある規則を除き、最初の確定error後に部分結果を保持しません。
