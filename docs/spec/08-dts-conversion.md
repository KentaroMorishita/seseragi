...
| `number`    | `Float`            | 既定。Intはsymbol override |
| `bigint`    | `BigInt`           | 任意精度を保持            |
| `never`     | `Never`            |                           |
...
TypeScriptの`number`だけからIntとFloatを区別できないため、既定はFloatです。Intを要求するsymbol / parameter /
resultはoverrideでIntを指定し、finite / integral / safe integer検査とnegative-zero正規化を行うhost number
adapterを生成します。TypeScriptの`bigint`は既定で`std/big-int`のBigIntへ変換し、Intへは変換しません。
...
TypeScript enumはforeign opaque typeとして生成します。module evaluationがpureなら各memberをforeign valueとして
bindできます。task-load moduleではmember取得がmodule loadを必要とするため自動value bindingを生成せず、task getterの
hand adapter requiredとしてreportします。数値enumをInt、文字列enumをStringへ弱めません。

## 8.11 unionとintersection

自動変換するunionはnullish unionだけです。それ以外は次の扱いです。

- string/number literal union: opaque foreign typeを生成する。module evaluationがpureなら、存在するexported constantも
  foreign valueとして生成できる。
...
task-load moduleのexported value、enum member、getterは、module評価なしに値を取得できないためpure valueへ自動変換
