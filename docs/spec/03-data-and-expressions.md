...
一度だけ評価します。Task の中身だけは `run` されるまで評価しません。
...
struct field は宣言 module の外からも読み取れます。生成を制限したい型は ADT と
smart constructor を使います。
...
comprehension は Array を返します。generator と guard を左から右へ処理します。
generator の source は `Iterable<A>` constraint を満たさなければなりません。
