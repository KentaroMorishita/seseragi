...
DOM mountを行いません。不正なsourceでも共通diagnosticと標準Referenceを返し、型やsymbolのqueryは得られた範囲だけを
...
| SES-N0002 | Error    | 同じscope / namespaceでnameが重複           |
...
| SES-T0503 | Error    | inherent `impl`のtargetまたは`self`が不正   |
| SES-T0504 | Error    | `impl`でoverloadできないoperatorを宣言      |
| SES-T0505 | Error    | `impl operator`のsignatureが標準traitと不一致 |
| SES-E0001 | Error    | Effect requirementまたはfailure型が不一致   |
| SES-K0001 | Error    | manifest / lockfileが不正または不一致       |
| SES-K0101 | Error    | manifest schemaまたは値が不正               |
...
| SES-F0103 | Error    | generated binding metadataがstale           |
...
missingまたは近似field、Effect必須位置のpure value、non-exhaustive match、parserのexpected token / expressionです。
これらは共通payloadのfixtureで固定し、surfaceごとにmessageを再構成しません。
