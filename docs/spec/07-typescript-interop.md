...
`= STRING` は現在のforeign namespaceにある一つのexactなown property keyです。`.`、`[`、`#`などをpath separatorと
して解釈せず、`"metrics.count"`はその文字列全体をkeyとするmemberだけを指します。prototype chainやdefault export
内部を暗黙探索しません。

nested host namespaceはforeign block内のcontextual `namespace`で表します。
...
local namespace名はmodule aliasと同じlower-nameです。`= STRING`を省略するとlocal spellingをhost keyに使い、指定時は
caseを含めexactに一致させます。namespaceは任意段数nestでき、各段を一つのown property lookupとして扱います。
`metrics.count`と`metrics.format.label`はSeseragiのqualified nameであり、任意値のfield accessではありません。
namespace外からnested memberをunqualifiedに参照できず、named import / re-exportは6章の公開namespace規則に従います。

namespace path lookupは各foreign call、constructor、property、value readの直前にrootから行います。task-load moduleでは
module load成功後かつTask実行中にだけlookupし、module importやTask構築時には行いません。segmentが存在しない、own
propertyでない、途中segmentがobject / functionでない、最終memberが要求call kindと一致しない場合はtask memberを
`Js.Error`のBindingLookup phaseで失敗させます。getter throwも同phaseで元causeを保持します。pure memberを含む
identityは従来どおりpure-loadで、lookupとgetterが同期的・非throw・決定的であることもbinding authorの保証です。

namespace functionはproperty callの暗黙receiverを渡さず、top-level functionと同じ`this = undefined`で呼びます。
receiverが必要なhost operationは`method fn`と明示receiverを使います。opaque typeだけのnamespaceはruntime lookupを
要求しません。`pure value`はpure-load binding初期化時にpathとmemberを一度読み、変換後の不変値を再利用します。
root moduleのload mode、single-flight、failure memoizationはnested memberを平坦化した全体で決めます。
...
phase: ModuleLoad | BindingLookup | SynchronousThrow | PromiseRejection | null
...
BindingLookupはwrapperがnamespace pathまたは最終member shapeを検証したgenerated interop frameをThrown groupへ
置き、lookup対象のhost property keyをmessageに含めます。getterがthrowした場合だけ元host frameを同groupへ続けます。
