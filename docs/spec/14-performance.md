...
相互再帰、部分適用されたself call、lambdaやcallbackの内側からの呼び出し、再帰結果へ演算を加えるnon-tail recursionには
一定stackを保証しません。Effect continuation、resource finalizer、`await`後の処理を残した呼び出しもtail positionでは
ありません。これらのstack exhaustionはdefectです。標準Effectの長いbind列、
...
shape testは内部IRのnode名やserializationを公開ABIにしません。fixtureはcanonical symbolと、次のsemantic
predicateを指定します。backendは任意のIRでpredicateを証明でき、証明できない場合はshape failureにします。

- `newtype-erased`: 指定symbol内で、newtypeのwrap / unwrapだけを理由とするallocation、tag、brand検査、runtime callが
  release outputに残らない。foreign ABIやvalidationを伴う明示境界は対象外です。
- `surface-sugar-erased`: 指定symbol内で、`$`、pipeline、`effect fn`、`do`そのものを表すruntime objectやdispatchが
  release outputに残らない。desugar後の関数callやEffect instructionは消去対象ではありません。
- `self-tail-loop`: 指定した直接自己再帰関数のすべてのtail edgeが、入力件数に比例してhost call stackを増やさない
  loopまたはtrampolineへlowerされる。非tail edgeがあるsymbolには成立しません。

predicateは「最適化を試みた」というcompiler eventではなく、release outputの性質です。source spellingやpass名を
