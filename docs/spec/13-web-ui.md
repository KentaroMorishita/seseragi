...
type ButtonType deriving Eq, Show =
  | Button
  | Submit
  | Reset

type InputType deriving Eq, Show =
  | TextInput
  | PasswordInput
  | CheckboxInput
  | RadioInput
...
snapshotはhost DOM Eventを保持せず、currentTarget、prototype、mutable fieldを公開しません。onInput / onChangeは
listener時点のtarget String valueを一度snapshotしてmapperへ渡します。
...
ButtonType absentは安全なButtonとして `type="button"`、InputType absentはTextInputとして `type="text"` を出します。
