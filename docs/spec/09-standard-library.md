...
`False < True`、StringはUnicode scalar列の辞書式順序、Unitは唯一の値をEqualとします。
...
Maybe、Either、EffectはApplicativeとMonadを持ちます。SignalはApplicativeを持ちますがMonadを
持ちません。Signalのpure/applyは5.13、動的dependency切替をtrait methodにしない理由は5.15に
従います。
