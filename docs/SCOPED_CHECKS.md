...
| Docs shell / content / Reference / selective client / quality pipeline | `bun run check:docs` | Docs lint/typecheck、canonical CLI build、route/input拒否、development/release出力一致、compiler-owned standard-library Referenceの全module/export、同じvalidated page dataからのsearch index、sitemap/robots、same-site page/asset/fragment integrity、page/client budgetと再現性。`apps/docs/scripts/browser.ts`で全routeをJS無効の3 viewport、heading/alt/metadata、search/copy success/failureをJS有効のdesktop/mobileで検証 |
| Docs production / deploy handoff | `SESERAGI_DOCS_ORIGIN=https://docs.example.com SESERAGI_DOCS_OUTPUT=/fresh/output bun run build:docs:production` | release CLI、release-profile generator/search、二重buildのcomplete manifest/file一致、strict deploy root、bytes/digests/runtime retentionを検証。`apps/docs/vercel.json`は別Vercel projectのprebuilt deploy configで、root Playground configを変更しない |
