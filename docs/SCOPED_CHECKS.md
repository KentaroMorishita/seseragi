...
| Docs shell / content / Reference / selective client pipeline | `bun run check:docs` | Docs lint/typecheck、canonical CLI build、route/input拒否、development/release出力一致、compiler-owned standard-library Referenceの全module/export、同じvalidated page dataからのsearch indexと再現性。`apps/docs/scripts/browser.ts`で全routeをJS無効の3 viewport、search/copyをJS有効のdesktop/mobileで検証 |
