...
| Effect、resource、concurrency              | 初稿あり      | lessonあり、境界fixture不足                | 未着手             |
| Signal、Stream                             | 初稿あり      | lessonあり、runtime fixture不足            | 未着手             |
| module、package、project                   | 初稿あり      | project schemaあり、module case不足        | 未着手             |
| TypeScript interop、`.d.ts`変換            | 初稿あり      | foreign load project 1件、snapshot不足     | 未着手             |
| collection、text、number、JSON             | 初稿あり      | lessonあり、境界fixture不足                | 未着手             |
| Bytes、Decimal、Regex、timezone            | 初稿あり      | lessonあり、fixture不足                    | 未着手             |
| filesystem、process、HTTP                  | 初稿あり      | filesystem lessonのみ、project fixture不足 | 未着手             |
| pure HTML、SSR、DOM、hydration             | 初稿あり      | pure HTML lessonとcompile fixtureあり      | 未着手             |
| 性能モデル、最適化境界                     | 初稿あり      | IR shape / benchmark未作成                 | 未着手             |
...
- conformance入力: positive 8件、diagnostic 5件
- multi-file conformance入力: project 1件
...
現在はsingle-file 13件、project 1件だけです。特に次が不足しています。
...
- module cycle、package解決、foreign binding、`.d.ts`変換のmulti-file project
- DOM reconciliation、event cleanup、hydration mismatchのproject fixture
- newtype erasure、tail loop、trait specialization、fusionのIR shape fixture
...
local recursion、test discovery、standard input、Random / secure entropy、traversalの短絡など、意図的な
不採用ではない未定義項目が横断監査で見つかっています。優先度と全一覧は`docs/SPEC_COVERAGE.md`の
「未定義・要設計」を正とし、frontend実装前に優先度Aを閉じます。
...
2. 性能境界を含むsemantic differentialとIR shapeのfixture形式を決める。
3. 各lessonへ最低1件のcompile fixtureを対応づける。
4. 失敗条件を持つ機能へdiagnosticまたはruntime fixtureを追加する。
5. module、interop、package、DOMをmulti-file project fixtureで固定する。
6. fixtureを通す最小compiler frontendとconformance runnerの実装計画を確定する。
7. parserから段階的に実装し、formatter、LSP、highlight、playgroundへ同じfrontendを接続する。
