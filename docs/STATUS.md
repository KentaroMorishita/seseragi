...
generic `Reducible<C,A>` dictionary method dispatch、struct / newtype内の`operator`糖衣、user-defined Eqは後続gateです。
...
2. project layerが`ModuleGraph`のtopological orderでdependency interfaceをcompileし、generated output pathから
   ...
   actual executionへ接続済み。次はgeneric `Reducible<C,A>`のparameter evidenceをdictionary method callへ接続し、
   struct / newtypeの`operator`糖衣とuser-defined Eqを同じ経路へ伸ばす。
