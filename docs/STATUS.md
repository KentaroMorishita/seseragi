...
- conformance入力: positive 21件、diagnostic 9件
...
現在はsingle-file 30件、project 41件です。特に次が不足しています。
...
standard API監査で`BigInt`の公開surface未定義を再発見し、`docs/SPEC_COVERAGE.md`の
「今回の基礎監査で残した未定義項目」へP1として登録しています。名前だけを実装へ推測させず、module APIと
arithmetic semanticsを正本で閉じてからfixtureへ移します。
