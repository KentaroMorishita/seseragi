...
`schema-1/user-add-operator`はoperand型からlocal `Add<Score, Score, Score>`を選択し、`+`を生成dictionaryの
`add` method callへlowerします。`project-schema-1/imported-user-add-operator`は同じ選択をmodule ABIへ伸ばし、
consumerがproviderのdictionary exportをimportしてactual executionするため、backendの型名・runtime tag分岐では
完了できません。対応instanceがない`Bool + Bool`は`semantic-diagnostics-schema-1/user-add-missing`で
`SES-T0201 instance.missing`になります。同じsourceはPlaygroundの`自作の足し算`から実行できます。
operator section `(+)`のpolymorphic constrained value、struct / newtype内の`operator`糖衣、user-defined Eqは後続gateです。
...
2. project layerが`ModuleGraph`のtopological orderでdependency interfaceをcompileし、generated output pathから
   ...
   Int算術とoperator sectionは`Add<Int, Int, Int>`等のevidenceを全IRへ保持済み。binary `+`はlocal / imported
   user-defined `Add<L, R, O>`のfunctional dependencyをoperandから解き、通常dictionary dispatchとactual executionへ接続済み。
   次はoperator sectionのpolymorphic constrained valueとstruct / newtypeの`operator`糖衣を同じ経路へ接続する。
