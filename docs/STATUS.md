...
残る言語側の主要gateは`reduce`のconstraint / instance dispatchと、operator sectionを`Add<L, R, O>` evidenceへ
一般化することです。constraintは`schema-1/constraint-arguments`でtrait名と型引数を全owned artifactへ保持済みですが、
dictionary selection / passingはまだ接続していません。
...
2. project layerが`ModuleGraph`のtopological orderでdependency interfaceをcompileし、generated output pathから
   ...
   接続済み。次はgeneric `reduce`のconstraint / instance dispatchをregistry / lockfile resolutionとは独立に回収する。
