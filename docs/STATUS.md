...
lockfile resolutionとArray / reduceを含むfull fixtureは後続Phase 2 gateとして残します。
...
2. project layerが`ModuleGraph`のtopological orderでdependency interfaceをcompileし、generated output pathから
   ...
   generated runtime executionと全source root identity auditまで接続済み。次はfull collection fixtureを一般的なlanguage sliceへ
   分解し、registry / lockfile resolutionとは独立に回収する。
