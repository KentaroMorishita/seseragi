...
generic higher-order parameter callも通常pipelineへ接続済みです。残る言語側の主要gateは`reduce`のconstraint / instance dispatchと
operator sectionです。
...
2. project layerが`ModuleGraph`のtopological orderでdependency interfaceをcompileし、generated output pathから
   ...
   immutable Array literalは全IRとgenerated codeへ接続済み。次はgeneric `reduce`をregistry / lockfile resolutionとは独立に回収する。
