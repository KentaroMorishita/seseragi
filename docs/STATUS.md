...
非Iterable sourceは`SES-T0201 instance.missing`で停止します。user-defined Iterable dictionary dispatchは後続gateです。
...
eta expansionによってdictionaryをその後ろへ渡します。未解決constraintを持つpolymorphic function valueは未接続です。
conditional imported dictionary selectionはdirect / transitive providerまで接続済みで、materializableなstandard `Show`
dictionaryもlocal generic factoryへ渡せます。残るgateはunresolved polymorphic constrained valueと、operation-only standard
traitをdictionaryとして公開する場合のABI設計です。
