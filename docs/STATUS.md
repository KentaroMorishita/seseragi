...
現在はsingle-file 26件、project 35件です。特に次が不足しています。
...
現時点で再登録されている未定義項目は、TypeScript namespace object内のruntime memberを指すforeign selectorです。
全一覧は`docs/SPEC_COVERAGE.md`の「今回の基礎監査で残した未定義項目」を正とし、frontend実装前に閉じます。
