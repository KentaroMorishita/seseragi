...
欠けた場合はlocal fallbackせずlowering errorになります。残るP2-4はgeneric / constraint付きfactoryの再帰materializationです。
...
concrete evidenceはdirect dependencyとfacade越しのprovider chainをexecutionまで通しますが、generic imported instance solverの
完了とは呼びません。P2-4の完了には、constraint付きfactoryのsubstitutionと再帰evidence materializationが必要です。
