...
このsliceの再帰materializationはlocal evidenceとinstance-level constraint scopeを対象にします。
standard / imported dictionary valueをfactory引数へ渡す経路は後続gateです。provider moduleを跨ぐ
constrained function callについては、後述のproject fixtureでconsumer-local evidenceまで接続します。
...
残るcoherenceの一般化、generic / constraint付きimported factory、first-class constrained value、standard evidenceの
factory引数化、transitive provider closureはまだ完了gateではありません。instance-levelとmethod-specific scoped evidence、
consumer-local evidence、provider dictionaryの三経路を同じcross-module ABIへ接続したうえで、Phase 3の一般trait /
instance goal programではlocalやdirect dependencyだけを通る経路と区別して残りを回収します。
...
このdirect concrete gateではgeneric imported instanceのsubstitution、constraint evidenceの再帰materialization、facade越しの
provider importは扱いません。それらとtransitive instance closureはP2-4の残りです。
...
module境界越しに運び、generated ESMのclosed type-checkと実行まで固定します。これはimported dictionary selectionを
証明するfixtureではなく、P2-4のtransitive instance closureは引き続き後続gateです。
...
argument identitiesからsemantic identityを確定します。direct dependencyのinstanceはResolvedAstに保持され、derived `Show`の
...
これはdirect dependency範囲のevidence transportであり、generic imported instance solverやtransitive instance closureの完了とは
呼びません。P2-4の完了には、constraint付きfactoryの再帰materializationとtransitive provider chainを実行まで通すgateが必要です。
