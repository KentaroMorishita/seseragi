...
first-classまたはimported constrained functionのevidence ABIと、standard / imported dictionary valueを
...
まだ完了gateではありません。instance-levelとmethod-specific scoped evidenceは接続済みですが、first-class /
imported constrained functionは追加のvalue scheme / cross-module ABIを要するため未接続です。それらはPhase 3の一般trait / instance goal programで、
...
このgateは直接または一つのexpression内で飽和するlocal constrained function callを対象にします。
partial applicationしたconstrained functionをlet / parameterなどfirst-class valueとして保持する場合は、
未解決constraintをvalue schemeへ保持する一般機構が必要です。imported constrained functionと
standard / imported dictionaryのparameter化はPhase 3の累積goal programで回収します。現在のlocal ABIを
それらの完了と数えません。
