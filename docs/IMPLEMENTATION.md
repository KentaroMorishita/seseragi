...
2026-07-11時点ではP1-2のうち、非再帰ADT、nullary / payload constructor、opaque export境界、
constructor値と`let` annotationの型検査、CoreIr / TypeScriptIr / tagged object生成を通常pipelineで通しています。
未知payload型と同一namespaceの重複constructorはresolver diagnosticで拒否します。generic payload constructorは
payloadから決まる型parameterをinstantiateできますが、nullary constructorの期待型からのcontextual specializationは
まだ未実装です。P1-3のmatch / exhaustivenessへ進む前提はここまでで満たし、generic contextual inferenceは独立sliceで
完成させます。
