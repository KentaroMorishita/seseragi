...
runtime dictionaryが実在するstandard `Show<String>`をfactory引数へ渡す経路は
...
materializableなstandard `Show<String>`もlocal generic factoryのconstraint argumentへ接続済みです。
残るcoherenceの一般化、first-class constrained value、operation-only standard traitのdictionary ABIは
...
partial trait methodは期待関数型からconcrete evidenceを選び、`partial-functor-value`でfirst-class valueとして保持できます。
一方、通常のconstrained top-level functionを未飽和のままlet / parameterへ保持する場合は、現在の
「全value parameterの後ろへdictionaryを渡す」ABIに対するeta expansionか、未解決constraintをvalue schemeへ保持する
...
top-level partial constrained valueを回収します。現在のtrait method partialだけを全体の完了と数えません。
...
evidenceをそのまま運びます。materializableなstandard `Show<String>`はsingle-fileのlocal generic factoryへ接続済みです。
P2-4の残りはtrait namespaceとfirst-class partial constrained valueであり、operation-only standard traitを
...
一方、trait namespaceとfirst-class partial constrained valueは未接続のため、
...
trait namespaceとfirst-class partial constrained valueが必要です。standard evidenceはruntime dictionaryが実在する
`Show<String>`だけをfactory引数へ接続し、operation-only traitをmaterialize済みとは数えません。
