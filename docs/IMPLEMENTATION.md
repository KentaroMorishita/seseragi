...
record literalとfield accessを独立nodeとして保持し、backendはreadonly object type / literal / indexed accessを生成します。
これによりnamespaceとmemberの判断をTypeScript shapeへ委譲せず、後続structでも同じfield backendを再利用できます。
このgateではoptional field accessを`undefined`へ黙って落とさず、`Maybe` semanticsが接続されるまでstructured diagnosticで
停止します。spreadとrecord patternも未実装であり、literal/accessだけをrecord全体の完了条件にはしません。
...
runtime `Show<String>`とlocal derived `Show<Badge>`の混在をactual outputまで固定し、Playgroundも同じsourceをbundleします。
