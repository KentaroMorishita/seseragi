...
imported instance search、coherence、first-class constrained value、standard / imported dictionary parameter passingは
まだ完了gateではありません。instance-levelとmethod-specific scoped evidence、およびimported constrained functionへ
consumer-local evidenceを渡すcross-module ABIは接続済みです。残るimported dictionary selectionはPhase 3の一般trait /
instance goal programで、標準型名やlocal instanceだけを通る経路と区別して回収します。
...
TypeScriptへ委譲しません。別moduleがexportしたdictionary自体の選択・importとtransitive instance closureはP2-4へ残します。
