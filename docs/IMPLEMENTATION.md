...
残るcoherenceの一般化、first-class constrained value、standard evidenceのfactory引数化は
...
未解決constraintをvalue schemeへ保持する一般機構が必要です。standard dictionaryのfactory引数化は
Phase 3の累積goal programで回収します。現在の飽和call ABIをその完了と数えません。
...
evidenceをそのまま運びます。残るP2-4 gateはstandard evidenceをfactory引数へ渡す経路です。
...
TypeScript source importまで接続済みです。一方、trait namespaceとstandard evidenceのfactory引数化は未接続のため、
