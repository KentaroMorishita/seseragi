...
import { intMul, intOne } from "../../../runtime/ts/src/int"
import {
...
describe("Collection runtime", () => {
  ...
  test("appends, concatenates, and reverses persistent List values", () => {
    ...
    expect(dropList(2, appended)).toBe(suffix)
    expect(
  ...
  test("finds and slices Array values with documented count boundaries", () => {
    ...
    expect(takeArray(-1, [1, 2, 3])).toEqual([])
    expect(takeArray(0, [1, 2, 3])).toEqual([])
    expect(takeArray(2, [1, 2, 3])).toEqual([1, 2])
    expect(takeArray(20, [1, 2, 3])).toEqual([1, 2, 3])
    expect(dropArray(-1, [1, 2, 3])).toEqual([1, 2, 3])
    expect(dropArray(0, [1, 2, 3])).toEqual([1, 2, 3])
    expect(dropArray(2, [1, 2, 3])).toEqual([3])
    expect(dropArray(20, [1, 2, 3])).toEqual([])
  })
  ...
  test("finds and slices persistent List values with shared suffixes", () => {
    ...
    expect(takeList(-1, values)).toBe(Empty)
    expect(takeList(0, values)).toBe(Empty)
    expect(takeList(2, values)).toEqual(fromArray([1, 2]))
    expect(takeList(20, values)).toEqual(values)
    expect(dropList(-1, values)).toBe(values)
    expect(dropList(0, values)).toBe(values)
    expect(dropList(2, values)).toBe(
      values.tag === "Cons" && values.tail.tag === "Cons"
        ? values.tail.tail
        : Empty
    )
    expect(dropList(20, values)).toBe(Empty)
  })
  ...
  test("observes Array values without leaking invalid indexes", () => {
    ...
    expect(lengthArray(values)).toBe(2)
    expect(isEmptyArray(values)).toBe(false)
    expect(isEmptyArray([])).toBe(true)
    expect(getArray(-1, values)).toBe(Nothing)
    expect(getArray(2, values)).toBe(Nothing)
    expect(getArray(1, values)).toEqual(Just(20))
    expect(headArray([])).toBe(Nothing)
    expect(headArray(values)).toEqual(Just(10))
    expect(tailArray([])).toBe(Nothing)
    expect(tailArray([10])).toEqual(Just([]))
    expect(tailArray(values)).toEqual(Just([20]))
  })

  test("observes persistent List values without leaking invalid indexes", () => {
    const values = fromArray([10, 20])

    expect(lengthList(values)).toBe(2)
    expect(isEmptyList(values)).toBe(false)
    expect(isEmptyList(Empty)).toBe(true)
    expect(getList(-1, values)).toBe(Nothing)
    expect(getList(2, values)).toBe(Nothing)
    expect(getList(1, values)).toEqual(Just(20))
    expect(headList(Empty)).toBe(Nothing)
    expect(headList(values)).toEqual(Just(10))
    expect(tailList(Empty)).toBe(Nothing)
    expect(tailList(fromArray([10]))).toEqual(Just(Empty))
    expect(tailList(values)).toEqual(Just(fromArray([20])))
  })
  ...
  test("multiplies from one and preserves the empty product", () => {
    expect(
      product<ReadonlyArray<number>, number>(arrayReducible, intOne, intMul, [
        2,
        3,
        4,
      ])
