...
var applyMaybe = (ff, fa) => ff.tag === "Just" && fa.tag === "Just" ? Just(ff.value(fa.value)) : Nothing;
var bindMaybe = (ma, f) => {
  return ma.tag === "Just" ? f(ma.value) : Nothing;
};
var isJust = (maybe) => maybe.tag === "Just";
var isNothing = (maybe) => maybe.tag === "Nothing";
var fromMaybe = (defaultValue, maybe) => maybe.tag === "Just" ? maybe.value : defaultValue;
...
var applyEither = (ff, fa) => ff.tag === "Right" && fa.tag === "Right" ? Right(ff.value(fa.value)) : ff.tag === "Left" ? ff : fa;
var bindEither = (ea, f) => {
  return ea.tag === "Right" ? f(ea.value) : ea;
};
var isLeft = (either) => either.tag === "Left";
var isRight = (either) => either.tag === "Right";
var fromLeft = (defaultValue, either) => either.tag === "Left" ? either.value : defaultValue;
var fromRight = (defaultValue, either) => either.tag === "Right" ? either.value : defaultValue;
