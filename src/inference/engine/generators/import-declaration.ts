...
import { generateConstraintsForMethodDeclaration, generateConstraintsForOperatorDeclaration } from "./struct-declaration"
...
export function generateConstraintsForImportDeclaration(
  ...
  for (const item of importDecl.items) {
    ...
    if (exportedFunction) {
      ...
      if (exportedType.kind === "TypeAliasDeclaration") {
        ...
        if (implBlock) {
          ...
          for (const operator of implBlock.operators) {
            generateConstraintsForOperatorDeclaration(ctx, operator, env, structType)
          }
        }
      } else {
...
function createFunctionTypeFromDeclaration(
  ...
  if (funcDecl.parameters.length === 0) {
    const unitType = new AST.PrimitiveType("Unit", funcDecl.line, funcDecl.column)
    return new AST.FunctionType(unitType, resultType, funcDecl.line, funcDecl.column)
  }
