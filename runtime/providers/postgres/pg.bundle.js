...
function createPostgresProvider(driver) {
  const pools = new Set;
    ...
    operations: {
      async openPool(value) {
        try {
          ...
          if (typeof options.connectionString !== "string" || !Number.isSafeInteger(options.maxConnections) || options.maxConnections <= 0) {
      ...
      async begin(value) {
        ...
        try {
          ...
          const token = { parent: pool, client };
      ...
      async transactionQuery(value) {
        try {
          ...
          return {
            kind: "success",
            value: queryResult(await transaction.client.query(query.text, query.values))
          };
        } catch (cause) {
      ...
      async fetch(value) {
        try {
          ...
          if (!Number.isSafeInteger(input.limit) || input.limit <= 0 || input.limit > 2147483647) {
            throw new RangeError("PostgreSQL cursor fetch limit is invalid");
          }
          const rows = await cursor.cursor.read(input.limit);
...
function ownedPool(value, pools) {
  if (typeof value !== "object" || value === null || !pools.has(value)) {
    throw new TypeError("PostgreSQL pool is not owned by this provider");
  }
  return value;
}
function ownedCursor(value, cursors) {
  if (typeof value !== "object" || value === null || !cursors.has(value)) {
    throw new TypeError("PostgreSQL cursor is not owned by this provider");
  }
  return value;
}
function ownedTransaction(value, pools) {
...
function ensureOpen(completion, name) {
  if (completion !== undefined)
    throw resourceClosed(`${name} is closed`);
}
...
function failure(operation, cause) {
  ...
  return {
    ...
    failure: Object.freeze({
      ...
      message: typeof error?.message === "string" ? error.message : "PostgreSQL failed"
    })
  };
}
function resourceClosed(message) {
...
var provider = createPostgresProvider({
  createPool: (config) => new esm_default.Pool({
    connectionString: config.connectionString,
    max: config.maxConnections
  }),
  openCursor(client, query) {
    return client.query(new esm_default2(query.text, query.values));
  }
});
export {
  provider
