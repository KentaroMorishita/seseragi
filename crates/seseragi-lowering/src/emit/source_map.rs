...
use crate::{
    ...
    regex_ops::runtime_regex_operation_for_feature,
    ...
    runtime_effect_operation_for_feature(feature)
        ...
        .or_else(|| {
            runtime_bytes_operation_for_feature(feature)
                ...
                .or_else(|| runtime_regex_operation_for_feature(feature))
                .map(|operation| operation.source_map_name)
                .or_else(|| {
        ...
        .or_else(|| {
            crate::stream_ops::runtime_stream_operation_for_feature(feature)
                .map(|operation| operation.source_map_name)
        })
        .or_else(|| {
        ...
        .or_else(|| {
            runtime_display_dictionary_for_feature(feature)
                .map(|dictionary| dictionary.source_map_name)
        })
        .or_else(|| {
            runtime_prelude_dictionary_for_feature(feature)
                .map(|dictionary| dictionary.source_map_name)
        })
        .or_else(|| {
        ...
        .or_else(|| {
            runtime_provider_service_operation_for_feature(feature)
                .map(|operation| operation.source_map_name)
        })
}
...
fn collect_expr_names(
    ...
    match expr {
        TypeScriptExpr::CheckedResult { value, .. } => {
            collect_expr_names(value, helper_names, names)
        }
        ...
        TypeScriptExpr::Lambda { body, .. } => collect_expr_names(body, helper_names, names),
