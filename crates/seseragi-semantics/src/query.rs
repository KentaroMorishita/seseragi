...
fn standard_description(identity: &str) -> Option<&'static str> {
    Some(match identity {
        ...
        "std/validation::Validation" => "Independent validation with a Valid value or a non-empty, source-ordered collection of errors.",
        "std/validation::Valid" | "std/validation::valid" => "Constructs a successful Validation value.",
        "std/validation::Invalid" | "std/validation::invalidMany" => "Constructs an invalid result from a NonEmptyList, preserving error order.",
