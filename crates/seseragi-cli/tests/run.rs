...
#[test]
fn runs_foreign_string_scalars() {
    let package = LockedProject::copy(
        &repository_root().join("examples/spec/fixtures/projects/foreign-string-scalars"),
    );
    let output = Command::new(env!("CARGO_BIN_EXE_seseragi"))
        .arg("run")
        .arg(&package)
...
#[test]
fn runs_callable_values() {
    let package = LockedProject::copy(
        &repository_root().join("examples/spec/fixtures/projects/callable-values"),
    );
    let output = Command::new(env!("CARGO_BIN_EXE_seseragi"))
        .arg("run")
        .arg(&package)
