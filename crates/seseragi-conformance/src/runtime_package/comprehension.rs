    ...
    let output = Command::new("bun")
        ...
        .arg(
            "import { collectFlatMap as arrayFlatMap, collectMap as arrayMap } from \"./src/array.ts\";\n\
             ...
             const evenSquares = rangeMap(inclusive(1, 10), (value) => value % 2 === 0, (value) => value * value);\n\
             const arrayPairs = arrayFlatMap([1, 2], () => true, (left) => arrayMap([10, 20], (right) => right > 10, (right) => left + right));\n\
             const rangePairs = rangeFlatMap(inclusive(1, 2), () => true, (left) => rangeMap(inclusive(3, 4), () => true, (right) => left * right));\n\
