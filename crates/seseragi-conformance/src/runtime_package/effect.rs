...
pub(super) fn check_effect_resource_boundary(root: &Path) -> Result<(), String> {
    let output = Command::new("bun")
        ...
        .output()
        .map_err(|error| format!("failed to run Effect resource probe: {error}"))?;
    if !output.status.success() {
        return Err(format!(
            "Effect resource probe failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        ));
    }
    if output.stdout != b"effect resource probe passed\n" {
        return Err(format!(
            "Effect resource probe returned unexpected output: {}",
            String::from_utf8_lossy(&output.stdout)
        ));
    }
    Ok(())
