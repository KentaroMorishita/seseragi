    ...
    let output = Command::new("bun")
        ...
        .arg(
            "import { collectFlatMap, collectMap, next, unfold } from \"./src/iterator.ts\";\n\
             ...
             let steps = 0;\n\
             const source = unfold((value) => { steps += 1; return value <= 2 ? Just([value, value + 1]) : Nothing; }, 1);\n\
             const before = steps;\n\
             ...
             const sourceForCollection = () => unfold((value) => value <= 3 ? Just([value, value + 1]) : Nothing, 1);\n\
             const mapped = collectMap(sourceForCollection(), (value) => value % 2 === 1, (value) => String(value * value));\n\
